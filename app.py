...
app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', 'sqlite:///' + os.path.join(os.path.dirname(os.path.abspath(__file__)), 'toolrental.db'))
        ...
        try:
            ...
            for i in range(1, 4):
                ...
                if image and allowed_file(image.filename):
                    ...
                    image_urls.append(os.path.join('tool_images', filename))  # Store the relative path
                else:
...
def delete_tool(tool_id):
    tool = Tool.query.get_or_404(tool_id)
    if tool.owner_id != session['user_id']:
        flash('You can only delete your own tools')
        return redirect(url_for('my_tools'))
    ...
    try:
        ...
        db.session.rollback()
        flash('Error deleting tool. Please try again.')
